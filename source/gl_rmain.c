	...
	if (distance_from_client >= 40000 || paliashdr->numframes <= 1) { // 200 * 200
		...
		if (r_lerpmodels.value && !(e->model->flags & MOD_NOLERP && r_lerpmodels.value != 2))
		{
			if (e->lerpflags & LERP_FINISH && numposes == 1)
				lerpdata->blend = CLAMP (0.0f, (cl.time - e->lerpstart) / (e->lerpfinish - e->lerpstart), 1.0f);
			else
				lerpdata->blend = CLAMP (0.0f, (cl.time - e->lerpstart) / e->lerptime, 1.0f);
...
void R_SetupEntityTransform (entity_t *e, lerpdata_t *lerpdata)
{
	...
	if (r_lerpmove.value && e != &cl.viewent && e->lerpflags & LERP_MOVESTEP)
	{
		if (e->lerpflags & LERP_FINISH)
			blend = CLAMP (0.0f, (cl.time - e->movelerpstart) / (e->lerpfinish - e->movelerpstart), 1.0f);
		else
			blend = CLAMP (0.0f, (cl.time - e->movelerpstart) / 0.1, 1.0f);
